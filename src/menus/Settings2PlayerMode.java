...
public class Settings2PlayerMode extends JFrame {
	...
	public Settings2PlayerMode(Object obj) {
		this.obj = obj;
		setSize(400, 600);
		setResizable(false);
		...
		pnlInput.setLayout(new GridLayout(8, 0));
		...
		JLabel lblName2 = new JLabel("Player 2:");
		...
		btnStart.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				checkSettings(name1, name2, sn, sm);
			}
		});
	...
	// validate settings
	private void checkSettings(String name1, String name2, String sn, String sm) {
		if (name1.isEmpty() || name2.isEmpty() || sn.isEmpty() || sm.isEmpty()) {
			JOptionPane.showMessageDialog(null, "You have to fill all Panels.");
		}
		else if (name1.equalsIgnoreCase(name2)) {
			JOptionPane.showMessageDialog(null, "You have to Enter differebt Names.");
		}
		else {
			int n = 0, m = 0;
			boolean done = false;
			...
			if (done) {
				if (n % 2 == 0 && n > 1 && n <= 20 && m >= 1 && m <= n - 1) {
					data = new GameSession(name1, name2, n, m);
					synchronized (obj) {
						obj.notifyAll();
					}
				}
					...
					JOptionPane.showMessageDialog(null, "N should be greater than 1.");
				}
				else if (m < 1){
					JOptionPane.showMessageDialog(null, "M should be greater than 0.");
				}else if (m > n-1){
					JOptionPane.showMessageDialog(null, "M should be less than N.");
				}
			}
		}
